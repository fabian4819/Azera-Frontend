...
export default function PicCampaignDetail() {
  const { id } = useParams();
  const [data, setData] = useState<DashboardData | null>(null);
  const [notFound, setNotFound] = useState(false);

  useEffect(() => {
    picApi.get(`/pic/campaigns/${id}/dashboard`)
  ...
  if (notFound) {
    return <p style={{ fontFamily: f, color: '#464652' }}>Campaign not found, or you no longer have access to it.</p>;
  }
  if (!data) return null;
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '12px', marginBottom: '20px' }}>
        <div>
          <h2 style={{ fontFamily: f, fontWeight: 800, fontSize: '1.3rem', color: '#191c20' }}>{data.campaign.name}</h2>
      ...
      <div style={{ background: 'white', borderRadius: '16px', border: '1px solid #e1e0ff', boxShadow: '0 2px 12px rgba(107,46,232,0.05)', overflow: 'hidden' }}>
        <div style={{ padding: '16px 14px 0' }}>
          <p style={{ fontFamily: f, fontWeight: 700, fontSize: '0.9rem', color: '#191c20' }}>
            Applicants & Progress ({rows.length})
          </p>
        </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', marginTop: '10px' }}>
            <thead>
              <tr>
                ...
                <th style={th}>Curation</th>
                <th style={th}>Apply Status</th>
                <th style={th}>Latest Submission</th>
                <th style={th}>Submission Status</th>
                <th style={th}>Total Revisions</th>
            ...
            <tbody>
              {rows.length === 0 ? (
                <tr><td colSpan={11} style={{ ...td, textAlign: 'center', color: '#777683', padding: '32px' }}>No applicants yet.</td></tr>
      ...
      <p style={{ fontFamily: f, fontSize: '0.72rem', color: '#8a8a99', marginTop: '14px', textAlign: 'center' }}>
        Read-only dashboard for this campaign's PIC & Handle-by. Data updates automatically.
      </p>
    </div>
  );
}
